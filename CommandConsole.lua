...
local KeysList = require("CommandConsole/KeysList")
local CommandList = require("CommandConsole/CommandList")

if not require("CommandConsole/KeysList") or not require("CommandConsole/CommandList") then
...
do
	...
	if not utils.file_exists(utils.get_appdata_path("PopstarDevs\\2Take1Menu\\luaconfig\\CommandConsole", "Default.ini")) then
		io.output(io.open(utils.get_appdata_path("PopstarDevs\\2Take1Menu\\luaconfig\\CommandConsole", "Default.ini"), "w"))
		io.write("")
		io.close(io.open(utils.get_appdata_path("PopstarDevs\\2Take1Menu\\luaconfig\\CommandConsole", "Default.ini"), "w"))
	end
end
...
menu.add_feature("Open Command Console", "toggle", MainParent.id, function(f)
	if utils.file_exists(utils.get_appdata_path("PopstarDevs\\2Take1Menu\\scripts\\CommandConsole", "CommandConsole.png")) then
		local CommandConsole = scriptdraw.register_sprite(utils.get_appdata_path("PopstarDevs\\2Take1Menu\\scripts\\CommandConsole", "CommandConsole.png"))
		...
		while f.on do
			...
			scriptdraw.draw_sprite(CommandConsole, v2(feature["X Position"].value / 100, feature["Y Position"].value / 100), feature["Box Size"].value, 0, ((255 & 0x0ff) << 0x00) | ((255 & 0x0ff) << 0x08) | ((255 & 0x0ff) << 0x10) | ((feature["Box Alpha"].value & 0x0ff) <<0x18))
			local input_string = ""
			if #input_key > 0 then
...
menu.add_feature("Save Settings", "action", MainProfileParent.id, function(f)
	if utils.file_exists(utils.get_appdata_path("PopstarDevs\\2Take1Menu\\luaconfig\\CommandConsole", "Default.ini")) then
		io.output(io.open(utils.get_appdata_path("PopstarDevs\\2Take1Menu\\luaconfig\\CommandConsole", "Default.ini"), "w"))
		io.write("")
		io.close(io.open(utils.get_appdata_path("PopstarDevs\\2Take1Menu\\luaconfig\\CommandConsole", "Default.ini"), "w"))
		io.output(io.open(utils.get_appdata_path("PopstarDevs\\2Take1Menu\\luaconfig\\CommandConsole", "Default.ini"), "a"))
		local savestring = ""
		for k, v in pairs(feature) do
			savestring = savestring .. tostring(k) .. "|" .. tostring(feature[k].value) .. "\n"
		end
		io.write(savestring)
		io.close(io.open(utils.get_appdata_path("PopstarDevs\\2Take1Menu\\luaconfig\\CommandConsole", "Default.ini"), "a"))
		menu.notify("Successfully saved settings!", Meteor, 3, 0x00ff00)
	end
end)
...
do
	if utils.file_exists(utils.get_appdata_path("PopstarDevs\\2Take1Menu\\luaconfig\\CommandConsole", "Default.ini")) then
		local file = io.open(utils.get_appdata_path("PopstarDevs\\2Take1Menu\\luaconfig\\CommandConsole", "Default.ini"), "r")
